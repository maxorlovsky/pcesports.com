    ...
    <div class="home">
        <div class="block tournaments-lists">
            <div class="block-content tournaments-to-watch">
                ...
                <loading v-if="tournamentsLoading" />

                <div class="event-list-wrapper">
                    <template v-if="watchTournaments.length">
                        <div v-for="game in watchTournaments"
                            :key="game.id"
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    >{{ game.name }}</h4>
                                    <div>
                                        <i class="fa fa-trophy" /> {{ game.prize }}
                                    </div>
                                </a>

                                <div class="watch-dates">
                                    <div><i class="fa fa-calendar" /> <i class="fa fa-step-forward" /> {{ game.startTime }}</div>
                                    <div><i class="fa fa-calendar" /> <i class="fa fa-times" /> {{ game.endTime }}</div>
                                </div>
            ...
            <div class="block-content tournaments-latest">
                ...
                <div v-if="upcomingTournaments.length"
                    class="event-list-wrapper"
                >
                    <event-item v-for="game in upcomingTournaments"
                        :key="game.id"
                        :game="game"
                    />
                </div>
            ...
            >
                ...
                >
                    ...
                    <router-link :to="'../events'"
                        class="view-all btn btn-lg btn-secondary"
                    >View all tournaments</router-link>
                </div>
            </div>
        </div>
...
const homePage = {
    ...
    created() {
        ...
        if (checkStorage) {
            ...
            axios.all([axios.get(`${pce.apiUrl}/tournaments/count`)])
                .then(axios.spread((tournamentsCountData) => {
                    for (const game of this.games) {
    ...
    methods: {
        ...
        parsePlayTournaments: function(tournamentsPlayData) {
            const gamesFiltered = tournamentsPlayData.data;
        ...
        parseWatchTournaments: function(tournamentsWatchData) {
            const gamesFiltered = tournamentsWatchData.data;
            const currentDate = new Date();
            const timezoneOffset = currentDate.getTimezoneOffset() * 60;
