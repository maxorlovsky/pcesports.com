        ...
        ?>
            <div class="<?=$k?> parent <?=strtolower($v['status'])?>">
                <a href="<?=_cfg('href')?>/<?=$v['link']?>" class="inner">
                    ...
                    <div class="timing">
                        <h4 class="timer hint" attr-time="<?=intval($v['time'] - time())?>" attr-msg="<?=$this->convertTime($v['time'], 'j M - '.($this->data->user->timestyle==1?'H':'h').':i'.($this->data->user->timestyle!=1?' A':null), 1)?>" <?=($k=='hss2'?'attr-br="1"':null)?>><img src="<?=_cfg('img')?>/bx_loader.gif" /></h4>
                    </div>
