...
class dota extends System
{
	...
	public function __construct($params = array()) {
		...
		$this->currentTournament = $this->data->settings['dota-current-number'];
	}
	...
	public static function getSeo() {
		$seo = new stdClass();
		$seo->title = 'DotA 2 tournaments';
        
        $u = new self;
        
        if (is_numeric($_GET['val2'])) {
            $seo->title = 'DotA 2 tournament '.strtoupper($u->server).'#'.$_GET['val3'];
            $seo->ogDesc = $seo->title;
        }
        
        $seo->ogImg = _cfg('img').'/footer-dota-logo.png';
