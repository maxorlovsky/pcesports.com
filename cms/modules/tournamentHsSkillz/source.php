...
class TournamentHsSkillz
{
	...
	public function fetchChat($form) {
        ...
        if ($form) {
            foreach($form as $v) {
                $fileName = $_SERVER['DOCUMENT_ROOT'].'/chats/ext_'.$v[1].'.txt';
	...
	public function sendChat($form) {
		$fileName = $_SERVER['DOCUMENT_ROOT'].'/chats/ext_'.$form[1].'.txt';
	...
	public function statusCheck($form) {
        ...
        if ($form) {
            foreach($form as $v) {
                ...
                $rows = Db::fetchRows('SELECT `id`, `online` FROM `participants_external` '.
    ...
    public function finishMatch($form) {
        ...
        if (_cfg('env') == 'prod') {
            $this->runChallongeAPI('tournaments/pentaclick-exths1/matches/'.$matchId.'.put', $apiArray);
        }
        else {
